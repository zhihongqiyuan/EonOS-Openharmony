...
/**
 * @brief free all resources used by an user_elf struct, including memory and pmos
 *
 * @param user_elf
 */
...
/**
 * @brief Load the remaining content of an ELF file according to metadata in elf_header,
 * and parse it into an user_elf struct.
 *
 * @param path [In]
 * @param elf_header [In] this function only borrow the pointer, and will not free it. It's
 * the caller's responsibility to control the life cycle of this pointer.
 * @param elf [Out] returning pointer to newly loaded user_elf struct if success. The ownership
 * of this pointer is transfered to the caller, so the caller should free it after use.
 * @return 0 if success, otherwise -errno is returned. All memory resources consumed by this
 * function are guaranteed to be freed if not success.
 */
...
/**
 * @brief Read content of an ELF file already stored in memory, and parse it into an user_elf struct.
 *
 * @param code [In] pointer to the memory range storing the ELF file. It's the caller's responsibility
 * to ensure the memory range is valid. Otherwise the behavior is undefined.
 * @param elf [Out] returning pointer to newly loaded user_elf struct if success. The ownership
 * of this pointer is transfered to the caller, so the caller should free it after use.
 * @return 0 if success, otherwise -errno is returned. All memory resources consumed by this
 * function are guaranteed to be freed if not success.
