...
struct launch_process_args {
     ...
     * transfer caps of 3 fundamental system server, i.e procmgr, fsm and lwip,
     * to the new process by default.
     */
    cap_t *caps;
    int nr_caps;
    /**
    ...
    int argc;
    /**
     * NULL-termiated array of arguments to be passed to the new process. It should
     * contain @argc valid parameters, otherwise the behavior is undefined.
     */
    char **argv;
    /**
     * Unique badge used to identify different processes by the ChCore kernel.
     */
    badge_t badge;
    int pid;
    /**
     * Address space ID for the new process, it should be unique to reduce TLB
     * flush when switching between two processes if possible.
     */
    u64 pcid;
    /**
     * Name of the new process, which would be stored in the kernel for debugging.
     */
    char *process_name;
