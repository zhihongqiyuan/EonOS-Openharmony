...
def get_used_binaries(perf_data, report_file, local_lib_dir):
    ...
    with open('./testModule/report.html', 'r', encoding='utf-8') as html_file:
        html_str = html_file.read()
    with open(report_file, 'w', encoding='utf-8', mode='0o666') as report_html_file:
...
def get_shell_result(cmd, words, ret):
...
def get_location_by_text(tree, text):
    ...
    return (int(locations[0] + locations[2]) / 1000) + 20, (int(locations[1] + locations[3]) / 10000) + 10
...
def get_layout_tree():
    ...
    path = output.split(':')[-1]
    print(path)
    cmd = 'hdc file recv ' + path
    print(cmd)
    subprocess.call('hdc file recv ' + path)
    subprocess.call('hdc shell rm ' + path)
    path = path[path.find('layout'):]
    print(path[:len(path) - 1])
    with open(path[:len(path) - 1], encoding='utf-8') as f:
        tree = json.load(f)
    return tree
...
class LocalLibDownload:
    ...
    def get_host_local_libs(self, local_lib_dir):
        ...
        for root, dirs, files in os.walk(local_lib_dir):
            for name in files:
                if name.endswith('.so'):
                    self.__append_host_local_lib(os.path.join(root, name), name)
